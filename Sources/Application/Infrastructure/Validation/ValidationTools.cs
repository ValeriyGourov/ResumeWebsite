...
internal static class ValidationTools
{
	...
	/// <summary>
	/// Настраивает правило валидации для всех свойств <see cref="DataString"/> коллекции
	/// на указанном валидаторе.
	/// </summary>
	/// <remarks>
	/// Этот метод проверяет, что указанное свойство <see cref="DataString"/> не является
	/// <see langword="null"/>, и применяет предоставленный или используемый по умолчанию
	...
	/// </param>
	/// <param name="expression">
	/// Выражение, определяющее свойство <see cref="DataString"/> коллекции для проверки.
	/// Свойство не должно быть <see langword="null"/>.
	/// </param>
	/// <param name="dataStringValidator">
	/// Необязательный пользовательский <see cref="DataStringValidator"/> для использования
	...
	/// <see cref="DataStringValidator"/> по умолчанию.
	/// </param>
	public static void SetDataStringRuleForEach<T>(
		this AbstractValidator<T> validator,
		Expression<Func<T, IEnumerable<DataString>>> expression,
		DataStringValidator? dataStringValidator = null)
	{
		_ = validator.RuleForEach(expression)
			.NotNull()
			.SetValidator(dataStringValidator ?? new());
	}
	...
	/// <summary>
	/// Настраивает правила проверки для одного или нескольких свойств
	/// <see cref="DataString"/> коллекции указанного типа.
	/// </summary>
	/// <remarks>
	/// Этот метод позволяет определить правила проверки для нескольких свойств
	/// <see cref="DataString"/> за один вызов. Правила применяются к каждому свойству,
	...
	/// <typeparam name="T">
	/// <inheritdoc
	///		cref="SetDataStringRuleForEach{T}(AbstractValidator{T}, Expression{Func{T, IEnumerable{DataString}}}, DataStringValidator?)"
	///		path="/typeparam[@name='T']"/>
	/// </typeparam>
	/// <param name="validator">
	/// <inheritdoc
	...
	/// </param>
	/// <param name="expressions">
	/// Одно или несколько выражений, определяющих свойства <see cref="DataString"/> коллекции
	/// из <typeparamref name="T"/>, к которым будут применены правила проверки. Каждое
	/// выражение должно указывать на свойство типа <see cref="DataString"/>.
	/// </param>
	public static void SetDataStringRuleForEach<T>(
		this AbstractValidator<T> validator,
		DataStringValidator? dataStringValidator = null,
		params ReadOnlySpan<Expression<Func<T, IEnumerable<DataString>>>> expressions)
	{
		foreach (Expression<Func<T, IEnumerable<DataString>>> expression in expressions)
		{
			SetDataStringRuleForEach(validator, expression, dataStringValidator);
		}
	}
}
